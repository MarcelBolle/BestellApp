...
function getTopDeleteBtn(item) {
    if (item.amount <= 1) return '';
    return `<button class="btn-icon-top" onclick="removeFromBasket('${item.name}', true)">
                <img src="./assets/icons/delete.svg" alt="Löschen">
            </button>`;
}

function getControlBtn(item) {
    if (item.amount === 1) {
        return `<button class="btn-icon" onclick="removeFromBasket('${item.name}', true)">
                    <img src="./assets/icons/delete.svg" alt="Löschen">
                </button>`;
    }
    return `<button class="btn-control" onclick="removeFromBasket('${item.name}')">-</button>`;
}